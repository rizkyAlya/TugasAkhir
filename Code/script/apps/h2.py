...
try:
    ...
    while True:
        ...
        try:
            ...
            _dt_path_cycle = (_dt_path_cycle + 1) & 0xFFFF
            if _dt_path_cycle == 0:
                _dt_path_cycle = 1
            try:
                gateway_client.write_register(
                    DT_PATH_PROBE_ADDR, int(_dt_path_cycle), unit=1
                )
                print(
                    f"DT_PATH_LAT,h2,{time.time():.6f},{_dt_path_cycle}",
                    flush=True,
                )
            except Exception as e:
