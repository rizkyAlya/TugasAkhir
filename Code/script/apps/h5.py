...
def run_mitm_attack(net, rtu_name=DEFAULT_RTU_NAME, gateway_name=DEFAULT_GATEWAY_NAME, attacker_name=DEFAULT_ATTACKER_NAME):
    ...
    # Enable API on gateway only when --mitm triggers this function.
    gateway.cmd("touch /tmp/h3_http_inject_enabled")
    ...
    attacker.cmd(
        ...
        f"  time.sleep(4)\" >> logs/host/{{ host.name }}.log 2>&1 & echo $! > /tmp/h5_http_inject.pid"
