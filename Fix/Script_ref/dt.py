...
Siklus: baca BRK_FB + P/Q -> update topologi -> update beban -> runpp ->
        proteksi arus line -> tulis V_DT + CMD ke gateway -> RTU -> field.
"""
import os
import time
...
GATEWAY_OPC = os.environ.get("GATEWAY_OPC", "opc.tcp://10.0.2.2:4840/mininet/")
...
def get_opcua_nodes(client):
    ...
    probe = root.get_child(["0:Objects", f"{idx}:SENSORS", f"{idx}:DT_path_probe"])
    return p_nodes, q_nodes, v_dt_nodes, brk_fb_nodes, cmd_nodes, probe
        ...
        try:
            ...
            for bus in range(1, NUM_BUS + 1):
                ...
                if line_idx is not None:
                    ...
                    i_note = f" I_line={i_line:.4f}kA"
                print(
                    f"  bus {bus}: BRK_FB={brk_fb[bus]} CMD={cmd[bus]} "
                    f"V_DT={net.res_bus.vm_pu.at[idx]:.4f} P={p_by_bus.get(bus, 0):.2f}{i_note}",
                    flush=True,
                )
